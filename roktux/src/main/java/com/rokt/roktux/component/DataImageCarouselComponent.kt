    ...
    override fun Render(
        ...
        Box(
            ...
            val wrapperContainer = modifierFactory.createContainerUiProperties(
                containerProperties = model.wrapperIndicator?.containerProperties,
                index = breakpointIndex,
                isPressed = isPressed,
            )
            Row(
                modifier = modifierFactory
                    .createModifier(
                        modifierPropertiesList = model.wrapperIndicator?.ownModifiers,
                        conditionalTransitionModifier = model.wrapperIndicator?.conditionalTransitionModifiers,
                        breakpointIndex = breakpointIndex,
                        isPressed = isPressed,
                        isDarkModeEnabled = isDarkModeEnabled,
                        offerState = offerState,
                    ...
                    .then(modifier)
                    .align(wrapperContainer.selfAlignmentBias?.let { BiasAlignment(0f, it) } ?: Alignment.BottomCenter)
